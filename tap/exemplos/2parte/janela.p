...
define fundo_preto(node_name1 , x1, y1 , x2 , y2);
  lvars
  ...
  BG_orig -> XptValue(janela_graf , XtN foreground);
  XpwDrawString(janela_graf , (x1*CHARWIDTH)+1 , (y1+1)*CHARHEIGHT , node_name1);
  FG_orig -> XptValue(janela_graf , XtN foreground);
enddefine;




define fundo_branco(node_name1, x1, y1 , x2 , y2);
 lvars
   node_name1 x1 y1  x2  y2 bck_colour frg_colour;
...
define tree_node_pressed(wid , item , press_release1);
  ...
  if (exacc ^int press_release1 ->> press_release1) >= 0  then
    ...
    then
       ...
       fundo_preto(node_name, x1, y1 , x2, y2);

       syssleep(30);

       fundo_branco(node_name , x1, y1 , x2, y2);
...
XptValue(janela_graf , XtN foreground) -> FG_orig;
XptValue(janela_graf , XtN background) -> BG_orig;

vars
  pixel_de_cor_verde;

0 -> pixel_de_cor_verde;
while not([% XpwQueryColor(janela_graf,pixel_de_cor_verde) %] = [0 255 0])
      and
      pixel_de_cor_verde < (2 ** 16 - 1)
      do
  1 + pixel_de_cor_verde -> pixel_de_cor_verde;
endwhile;


if pixel_de_cor_verde < 2 ** 16 -1 then
  pixel_de_cor_verde -> XptValue(janela_graf , XtN background);
endif;
