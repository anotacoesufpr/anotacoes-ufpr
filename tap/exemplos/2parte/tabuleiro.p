...
defmethod observe_o_pressionar_do_botao_esquedo_do_mouse_com(funcao, identificador_partida);
  define clique_dado_na_posicao(id_janela, funcao_e_parametros, press_release1);
    lvars id_janela funcao_e_parametros press_release1 funcao uma_partida_do_jogo;
    if (exacc ^int press_release1 ->> press_release1) = 1  then
      funcao_e_parametros(1) -> funcao;
      funcao_e_parametros(2) -> uma_partida_do_jogo;
      funcao(XptValue(id_janela, XtN mouseX), XptValue(id_janela,XtN mouseY), uma_partida_do_jogo);
    endif;
  enddefine;
  ;;; XtRemoveAllCallbacks(identificador_partida<-superficie_grafica, XtN buttonEvent);
  XtAddCallback(identificador_partida<-superficie_grafica, XtN buttonEvent, clique_dado_na_posicao, [ ^funcao ^identificador_partida]);
enddefmethod;

defmethod suspende_observacao_de_eventos_do_mouse_em(identificador_partida);
  XtRemoveAllCallbacks(identificador_partida<-superficie_grafica, XtN buttonEvent);
enddefmethod;
...
defmethod escreve_na_posicao(valor, x, y);
  if isnumber(valor) then
    '' >< round(valor) -> valor;
  endif;
  XpwDrawString(superficie_grafica, round(x), round(y), valor);
enddefmethod;
