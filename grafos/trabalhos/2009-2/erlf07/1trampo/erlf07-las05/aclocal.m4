...
dnl
dnl AC_PATH_GTKGL([ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND]])
dnl
AC_DEFUN(AC_PATH_GTKGL,
  [
    ...
    AC_LANG_SAVE
    AC_LANG_C
    AC_CHECK_LIB(gtkgl-2.0, gdk_gl_query,
      [
        ac_save_CPPFLAGS="${CPPFLAGS}"
        CPPFLAGS="${CPPFLAGS} ${GTK_CFLAGS} ${gtkgl__Idir}"
        AC_CHECK_HEADER(gtkgl-2.0/gtkgl/gtkglarea.h,
          [
            have_gtkgl=yes
...
dnl Put dependencies in Makefile
AC_DEFUN(MAKE_GTK_DEPS, [
    echo -n "checking dependencies... "
    for i in *.c; do
        ${CC-cc} -w -MM $i `pkg-config --cflags gtk+-2.0` 1>> Makefile;
    done
    for i in iface/*.c; do
        echo -n "iface/" >> Makefile;
        ${CC-cc} -w -MM $i `pkg-config --cflags gtk+-2.0` 1>> Makefile;
    done
    echo "ok"
])
