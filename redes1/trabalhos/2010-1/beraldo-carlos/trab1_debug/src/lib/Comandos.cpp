...
{
	//cout << "tentando fazer CD para " << path << endl;
...
bool Comandos::Append( string filename, string dado )
{
	...
	cout << "\n\nEscrevendo no arquivo:" << endl;
	cout << "dado: " << dado << endl;
	cout << "dado.c_str: " << dado.c_str() << endl;
	fprintf( fp, "%s\n", dado.c_str() );
...
bool Comandos::Edit( string filename, int linhaEditada, string dado )
{
	...
	cout << "vamos renomear " << nomeArqTemp.c_str() << " para " << filename.c_str() << endl;
 ...
 * Esse método é chamado pelo cliente, para o qual todos os comandos são remotos
 */
void Comandos::ExecutarComandoRemoto( string comando, Conexao *Con )
{
	Pacote *pack = new Pacote();
	char cmd;

	// como é início de envio, a Sequencia é zero
	pack->setSequencia( 0 );


	/* Procura pela constante referente ao comando digitado */
	// coloca em "cmd" o comando digitado, sem parâmetros
	istringstream s_comando( comando );
	s_comando >> cmd;
	...
	//Protocolo::EnviarString( Con, comando, pack );
}
 ...
 * Esse método é chamado pelo servidor, para o qual todos os comandos são locais
 *
 * @return Retorna a saída do comando
 */
string Comandos::ExecutarComandoLocal( string comando )
{
	char cmd; // o char que representa o comando
	string param; // parâmetro do comando, se houver
	int idComando;

	ostringstream saidaComando; // stream que receberá a saída do comando
	...
	idComando = this->IDComando( cmd );

	// se huover parâmetros
	if ( !cin.eof() )
	{
		s_comando >> param;
	}
	...
	{
		case TIPO_LS:
			string comandoFormatado = "ls " + param;
			char buffer[ TAM_DADOS ];

			cout << "Vamos executar : " << comandoFormatado.c_str() << endl;

			FILE *fp = popen( comandoFormatado.c_str(), "r" );
			...
			break;
		/*default:
			cerr << "Comando com ID '" << idComando << "' não etsá presente no switch" << endl;
			break;*/
	}
