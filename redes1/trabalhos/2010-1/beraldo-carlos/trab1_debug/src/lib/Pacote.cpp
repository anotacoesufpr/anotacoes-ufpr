...
bool Pacote::String2Pacote( uchar * mensagem, Pacote *pack )
{
	...
	if ( mensagem[0] != MARCA )
	{
		//cerr << "O pacote não é iniciado pela MARCA" << endl;
		//cerr << "mensagem[0] = " << mensagem.at( 0 ) << endl;
		return false;
	}
	...
	{
		//cout << "dado.size == 0. string vazia para o dado" << endl;
		dado.operator += ( "" );
	}

	pack->setDado( dado );

	printf( "Recebido: 0x%x | %s\n\n", pack->getTipo(), pack->getDado().c_str() );
